 ...
 *    -------------   -------------------    ---------------------------------------------------------
 *    11Apr2025        thebearmay            v0.0.1 - Original code
 */
...
definition (
	name: 			"ZwaveJS Re-Interview", 
	namespace: 		"thebearmay", 
	author: 		"Jean P. May, Jr.",
	description: 	"Little app to reinterview all Zwave Nodes",
	category: 		"Utility",
	importUrl: "https://raw.githubusercontent.com/thebearmay/hubitat/main/apps/zwInterview.groovy",
    installOnOpen:  true,
	oauth: 			true,
    iconUrl:        "",
    iconX2Url:      ""
...
def configPage(){
    dynamicPage (name: "configPage", title: "<h2 style='background-color:#e6ffff;border-radius:15px'>${app.getLabel()}<span style='font-size:xx-small'>&nbsp;v${version()}</span></h2>", install: true, uninstall: true) {
        section (name:'cPageHndl', title:''){
            zwaveJsStat()
            String db = getInputElemStr(name:'debugEnabled', type:'bool', width:'15em', radius:'12px', background:'#e6ffff', title:'<b>Debug Enabled</b>')
			...
			String timeBtwn = getInputElemStr(name:'secBtwn', type:'number', width:'15em', radius:'12px', background:'#e6ffff', title:'<b>Seconds between Interviews</b>', defaultValue:1)
            String intvwBtn = getInputElemStr(name:'startIntBtn', type:'button', width:'15em', radius:'12px', background:'#e6ffff', title:'<b>Start</b>')			
            if(!state.onZwJs)
				intvwBtn = "ZWaveJS not Detected"
            paragraph "<table><tr><td style='min-width:15em'>${db}</td><td style='position:relative;top:0px;padding:0px;margin:0px'>${aRename}</td></tr><tr><td>${timeBtwn}</td><td>${intvwBtn}</td></tr></table>"
            if(nameOverride != app.getLabel()) app.updateLabel(nameOverride)
            if(state.startIntvw) {
                paragraph "Starting Interviews"
            	state.startIntvw = false
                interviewNodes()
                paragraph "Interviews Completed"
            }
        }
    }
}
...
ArrayList getNodes(){
    ...
    httpGet(params) { resp ->
        resp.data.nodes.each {
            nodeList.add(it.nodeId)
        }
    }
...
void interviewNodes(){
    ...
    log.info "Starting ZW Interviews"
    if(!timeBtwn || timeBtwn < 1)
    	pTime = 1000
    else 
        pTime = timeBtwn * 1000
	nList = getNodes()
    nList.each {
        log.info "Interviewing ZW Node ${it}"
        doInterview(it.toInteger())
		pauseExecution(pTime)
    }

    state.interviewing = false
    log.info "ZW Interviews Complete"
}

void doInterview(nodeId) {
	params = [
        uri    : "http://127.0.0.1:8080/hub/zwave2/reinterview?node=${nodeId}",
    ]
    httpGet(params) { resp -> }
