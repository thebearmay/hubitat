...
definition (
	name: 			"Device Details Display v2", 
	namespace: 		"thebearmay", 
	author: 		"Jean P. May, Jr.",
	description: 	"Display the capabilities, attributes, commands and device data for devices selected.",
	category: 		"Utility",
	importUrl: "https://raw.githubusercontent.com/thebearmay/hubitat/main/apps/deviceCharacteristicsv2.groovy",
	oauth: 			false,
    iconUrl:        "",
    iconX2Url:      ""
) 
...
def deviceReport(){
    dynamicPage (name: "deviceReport", title: "", install: false, uninstall: false) {
	  section("Device Characteristics Report"){
        ...
        """
         paragraph "$rptHTML"
         paragraph "<style>div{overflow:auto}</style>$rpt"
      }
    }
}
...
def buildReport() {
    ...
    String html = "<style>th, td{text-align:left;border:solid 1px blue}</style><table>"
    html += "<tr><th>Name</th><th>Type</th><th>Controller</th><th>Attributes</th><th>Data</th><th>DNI</th><th>Last Activity</th></tr>"
    ...
    qryDevice.each {
        ...
        it.currentStates.each{
            attrString += "<b>Name:</b>${it.name} <b>Value:</b>${it.value} <b>Type:</b>${it.dataType}<br>"
        }
        ...
        dData.each{
            if(it.key == "ip" && it.key.indexOf(".") == -1){
                ...
                dString += "<b>${it.key}</b> ${it.value}<br>"
        }            
        html += "<td>$dString</td>"
