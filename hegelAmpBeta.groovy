...
def syncState(message){
	...
	switch msgCmd{
		case "i":
            ...
            updateAttr("input", "${iVals[(Integer)inputNum-1]}","","Set by sendMsg")
			break
		case "m":
		    if(msgParam == "t"){
		...
		case "v":
		    if (msgParam == "u")
			    ...
			    updateAttr("lastVolume", device.currentValue("lastVolume").toInteger() 1 1,"","Set by sendMsg")
			else 
