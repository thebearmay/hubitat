...
definition (
	name: 			"Virtual Device Clone", 
	namespace: 		"thebearmay", 
	author: 		"Jean P. May, Jr.",
	description: 	"Read and Clone Devices",
	category: 		"Utility",
	importUrl: "https://raw.githubusercontent.com/thebearmay/hubitat/main/apps/vDeviceClone.groovy",
    installOnOpen:  true,
	oauth: 			false,
    iconUrl:        "",
    iconX2Url:      ""
...
def configPage(){
    dynamicPage (name: "configPage", title: "<h2 style='background-color:#e6ffff;border-radius:15px'>${app.getLabel()}<span style='font-size:xx-small'>&nbsp;v${version()}</span></h2>", install: true, uninstall: true) {
        section (name:'cPageHndl', title:'Configuration Page'){
            ...
            String sDev = getInputElemStr(name:'selectedDev', type:'capability.*', width:'15em', radius:'12px', background:'#e6ffff', title:'<b>Select Device to Clone</b>')
			...
			String numClones = getInputElemStr(name:"cloneAmt", type:"number", title: "<b>Number of Clones to Create</b>", defaultValue: "${settings['cloneAmt']}", width:'15em', radius:'12px', background:'#e6ffff')
            String goBtn = getInputElemStr(name:"cloneBtn", type:"button", title: "<b>Clone Now</b>", width:'15em', radius:'12px', background:'#e6ffff')
            if (!selectedDev.controllerType) {
	            if(state.cloneNow || state.cloneNow == 'true') {
    	            msg = 'Button Pushed'
        	        msg = cloneDevice()
            	    state.cloneNow = false
                } else {
            	    msg = ' '
                }
            } else {
                state.cloneNow = false
                msg = "Invalid Device Controller Type = ${selectedDev.controllerType}"
            }
            
            
            paragraph "<table><tr><td style='min-width:15em'>${sDev}</td><td>${numClones}</td></tr><tr><td>${goBtn}</td></tr><tr><td>${msg}</td></tr><tr><td></td></tr><tr><td>${aRename}</td></tr></table>"
...
String cloneDevice() {
	...
	this.installedDrivers.each{
   	    if(it.name == selectedDev.typeName) nSpace = it.namespace
    }
	params = [
        uri    : "http://127.0.0.1:8080",
        path   : "/device/fullJson/${selectedDev.id}",
        headers: [
    ...
    for(iteration=0;iteration<cloneAmt.toInteger();iteration++){
    	if(selectedDev.label)
    		nDev = addChildDevice("${nSpace}","${selectedDev.typeName}","${selectedDev.deviceNetworkId}-${iteration}}",["name":"${selectedDev.name}-${iteration}","label":"${selectedDev.label}-${iteration}"])
		else
        	nDev = addChildDevice("${nSpace}","${selectedDev.typeName}","${selectedDev.deviceNetworkId}-${iteration}}",["name":"${selectedDev.name}-${iteration}"])
        selectedDev.data.each{
            nDev.updateDataValue("${it.key}","${it.value}")
        }
        selectedDev.supportedAttributes.each{
            nDev.sendEvent(name:"$it",value:"${selectedDev.currentValue("$it")}")
        }

        jString.settings.each{
            nDev.updateSetting("${it.name}",[value:"${it.value}",type:"$it.type"])
        }
    }    
	return "<p>${iteration} device(s) created</p>"
}
