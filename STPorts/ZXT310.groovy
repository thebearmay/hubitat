 ...
 *
 *   16Mar2022    thebearmay     Port to HE 
 *
 */
metadata {
...
def autoOff() {
	...
	def cmds = []
	off()?.each {
		cmds << new hubitat.device.HubAction(it)
	}
	if (cmds) {
		sendHubCommand(cmds)
	}	
...
def zwaveEvent(hubitat.zwave.commands.configurationv2.ConfigurationReport cmd, ep=null) {
