 ...
 */
metadata {
    definition(name: "WeMo Local Dimmer", namespace: "local.wemo.v2", author: "OpenAI") {
        capability "Switch"
        capability "Refresh"
...
def setLevel(value, duration=null) {
    Integer level=Math.max(1,Math.min(100,value as Integer))
    soap("SetBinaryState",[BinaryState:"1",brightness:level.toString(),fader:"0"],"parseState")
}
...
def parseState(response) {
    try {
        ...
        def l = body =~ /<brightness>([^<]+)<\/brightness>/
        if (l.find()) sendEvent(name:"level", value:Math.max(0,Math.min(100,l.group(1).toInteger())))
