 ...
 *    20Sep23    thebearmay    change reboot resync handler 
*/
...
void getResp(resp, data) {
    try {
        ...
        if(data['cmd'] == 'resync'  && resp.data.substring(0,1) == '<') //remote hub returned a web page (it is rebooting), resend sync request
