...
definition (
	name: 			"Hub Notification Forwarder", 
	namespace: 		"thebearmay", 
	author: 		"Jean P. May, Jr.",
	description: 	"Forward all notifications recieved by the collector device to another hub.",
	category: 		"Utility",
	importUrl: "https://raw.githubusercontent.com/thebearmay/hubitat/main/apps/notifyForward.groovy",
	oauth: 			true,
    installOnOpen:	true,
    iconUrl:        "",
    iconX2Url:      ""
...
def mainPage(){
    dynamicPage (name: "mainPage", title: "", install: true, uninstall: true) {
        section (name:'cPageHndl', title:'Configuration Page'){
        	...
        	String pStr = "<div id='apiSection' ${divStyle}><p><b>Local Server API:</b> ${getFullLocalApiServerUrl()}/newNotice?access_token=${state.accessToken}</p>"
        	pStr+="<p><b>Cloud Server API: </b>${getFullApiServerUrl()}/newNotice?access_token=${state.accessToken}</p>"
        	pStr+="<p><b>Access Token: </b>${state.accessToken}</p></div>"

        	paragraph "${apiSection}${pStr}"

            String sDev = getInputElemStr(name:'selectedDev', type:'device.NotificationCollector', width:'15em', radius:'12px', background:'#e6ffff', title:'<b>Select Notification Collector Device</b>')
            String nDev = getInputElemStr(name:'outputDev', type:'capability.notification', width:'15em', radius:'12px', background:'#e6ffff', title:'<b>Select Notification Output Device</b>',multiple:'true')
            String ph = getInputElemStr(name:'primaryHub', type:'text', width:'15em', radius:'12px', background:'#e6ffff', title:'<b>Enter Primary Hub API String</b>')
            ...
            paragraph "<table><tr><td style='min-width:15em'>${db}</td><td>${ph}</td><td>${aRename}</td></tr><tr><td>${sDev}</td><td>${nDev}</td></tr></table>"
			if(selectedDev) 
				subscribe(selectedDev,"lastNotification",forwardNotice)
            if(nameOverride != app.getLabel()) app.updateLabel(nameOverride)
        }
    }
}

void forwardNotice(evt){
    HashMap nVal = [:]
    nVal.put("hubName","${location.hub.name}")
    nVal.put("notification","${evt.value}")
...
def newNotice(){
    ...
    if(outputDev){
        String notifyStr = "${jsonData.hubName}: ${jsonData.notification}"
	    outputDev.each { 
