 ...
 */
metadata {
    definition(name: "WeMo Local Insight", namespace: "local.wemo.v2", author: "OpenAI") {
        capability "Switch"
        capability "Refresh"
        capability "EnergyMeter"
        capability "PowerMeter"
        attribute "energy_today", "number"
        attribute "power_factor", "number"
...
def refresh() {
    soap("GetBinaryState", [:], "parseState")
    soap("GetInsightParams", [:], "parseInsight")
}
...
def parseInsight(response) {
    try {
        String body=response?.body ?: response?.data?.toString() ?: ""
        // WeMo Insight commonly returns InsightParams as a pipe-delimited value:
        // state|todayEnergy|totalTime|todayTime|currentPower|...
        def m=body =~ /<InsightParams>([^<]+)<\/InsightParams>/
        if (m.find()) {
            def a=m.group(1).split("\\|",-1)
            if (a.size()>4) {
                sendEvent(name:"power", value:Math.max(0,a[4].toBigDecimal()))
            }
            if (a.size()>1) sendEvent(name:"energy_today", value:Math.max(0,a[1].toBigDecimal()))
        }
        sendEvent(name:"connection",value:"connected")
    } catch(e) { logDebug("Insight parse: ${e.message}") }
}
